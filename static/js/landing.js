...
(function () {
  ...
  /* ============================================================
     ...
     ============================================================ */
  var compare = document.getElementById('compareWidget');
  if (compare) {
    var view = compare.querySelector('.compare-view');
    var fg = compare.querySelector('.compare-foreground');
    ...
    function setPos(clientX) {
      ...
      // foreground width reveals the right-side image
      fg.style.width = pct * 100 + '%';
    ...
    function onDown(e) {
      dragging = true;
      e.preventDefault();
      setPos(e.clientX !== undefined ? e.clientX : e.touches[0].clientX);
    }
    function onMove(e) {
      if (!dragging) return;
      setPos(e.clientX !== undefined ? e.clientX : e.touches[0].clientX);
    ...
    handle.addEventListener('mousedown', function (e) { e.stopPropagation(); onDown(e); });
    window.addEventListener('mousemove', onMove);
    window.addEventListener('mouseup', onUp);

    view.addEventListener('touchstart', onDown, { passive: false });
    handle.addEventListener('touchstart', function (e) { e.stopPropagation(); onDown(e); }, { passive: false });
    ...
    view.setAttribute('aria-valuenow', '50');
    var pct = 50;
    fg.style.width = pct + '%';
    handle.style.left = pct + '%';
    view.addEventListener('keydown', function (e) {
      if (e.key === 'ArrowLeft') {
        pct = Math.max(0, pct - 5);
        fg.style.width = pct + '%';
        handle.style.left = pct + '%';
        view.setAttribute('aria-valuenow', String(pct));
        e.preventDefault();
    ...
    // pause page scroll while dragging vertically over the widget on touch
    view.addEventListener('touchmove', function (e) {
      // only preventDefault while actively adjusting the slider
